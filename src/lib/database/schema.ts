...
async function apply(db: Transaction<any>, { schema }: CRSchema) {
	const logger = {
		log: (...params: any[]) =>
			console.log("[crstore schema apply]", ...params),
	};
	logger.log("started");
	...
	for (const table in schema) {
		...
		for (const column in current.schema) {
			...
			query = query.addColumn(
				column,
				current.ordered?.find(([x]) => x === column)
					? "blob"
					: covert(type),
				(col) =>
					current.primary?.includes(column) ? col.notNull() : col,
			);
		}
		// Add constrains
		if (current.primary) {
		...
		logger.log(`[table: ${table}]`, "creating...");
		await query.execute();
		logger.log(`[table: ${table}]`, "created!");

		// Create indices
		logger.log(`[table: ${table}]`, "creating indices...");
		...
		logger.log(`[table: ${table}]`, "created indices!");

		// Register CRRs
		//
		// NOTE: DEBUG: this is where the initalisation stops
		logger.log(`[table: ${table}]`, "registerind crrs...");
		if (current.crr) {
			await sql`SELECT crsql_as_crr(${table})`.execute(db);
		}
		logger.log(`[table: ${table}]`, "registered crrs!");

		// Register fraction index
		logger.log(`[table: ${table}]`, "registerind frac indices...");
		for (const ordered of current.ordered || []) {
			await sql`SELECT crsql_fract_as_ordered(${table},${sql.join(
				ordered,
			)})`.execute(db);
		}
		logger.log(`[table: ${table}]`, "registered frac indices!");

		// Create a special table for version sync
		logger.log(`[table: ${table}]`, "creating version sync table...");
		await db.schema
    ...
    `.execute(db);
		logger.log(`[table: ${table}]`, "created version sync table!");
	}
}
